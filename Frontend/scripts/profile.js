...
function setupEventDelegation() {
  const profileEndpointsList = document.getElementById("profileEndpointsList");

  // Remove any existing listeners to prevent duplicates
  const newProfileEndpointsList = profileEndpointsList.cloneNode(false);
  profileEndpointsList.parentNode.replaceChild(
    newProfileEndpointsList,
    profileEndpointsList
  );

  // Add single click handler using event delegation
  newProfileEndpointsList.addEventListener("click", (e) => {
    ...
    if (editBtn) {
      ...
      console.log("Edit clicked for:", endpointId);
      openEditModal(endpointId);
    } else if (deleteBtn) {
      e.preventDefault();
      e.stopPropagation();
      const endpointId = deleteBtn.getAttribute("data-endpoint-id");
      console.log("Delete clicked for:", endpointId);
...
function updateUserInfo() {
  ...
  const email = SecurityUtils.sanitizeHTML(currentUser.email);

  UIUtils.setTextContent("sidebarAvatar", initial);
  UIUtils.setTextContent("sidebarUsername", username);
  UIUtils.setTextContent("profileAvatar", initial);
  UIUtils.setTextContent("profileUsername", username);
  UIUtils.setTextContent("profileEmail", email);
}

// Update profile stats
function updateProfileStats() {
  const total = endpoints.length;
  UIUtils.setTextContent("profileTotalEndpoints", total.toString());

  const onlineCount = endpoints.filter((e) => e.status === "online").length;
  UIUtils.setTextContent("profileOnlineEndpoints", onlineCount.toString());

  if (total > 0) {
    const avgUptime =
      endpoints.reduce((sum, e) => sum + parseFloat(e.uptime), 0) / total;
    UIUtils.setTextContent("profileAvgUptime", avgUptime.toFixed(1) + "%");
  } else {
    UIUtils.setTextContent("profileAvgUptime", "0%");
  }
}
...
addEndpointForm.addEventListener("submit", async (e) => {
  ...
  const newEndpoint = { name, url, interval: selectedInterval };
...
editEndpointForm.addEventListener("submit", async (e) => {
  ...
  const updatedEndpoint = { name, url, interval: editSelectedInterval };
...
function renderEndpoints() {
  ...
  if (endpoints.length === 0) {
    profileEndpointsList.innerHTML =
      '<p style="color: var(--text-secondary); text-align: center; padding: 2rem;">No endpoints yet. Add one to get started!</p>';
    return;
  }

  const html = endpoints
    .map((endpoint) => {
      ...
      const safeInterval = SecurityUtils.sanitizeHTML(
        String(endpoint.interval)
      );
      ...
      return `
            <div class="endpoint-card" data-endpoint-id="${endpointId}">
                ...
                <div class="endpoint-stats">
                    ...
                    <div class="endpoint-stat">
                        <div class="endpoint-stat-label">Interval</div>
                        <div class="endpoint-stat-value">${safeInterval}s</div>
                    </div>
                </div>
            </div>
        `;
  ...
  profileEndpointsList.innerHTML = html;
}
